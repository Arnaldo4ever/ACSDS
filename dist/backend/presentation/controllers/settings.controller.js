...
export class SettingsController {
    async runSettingsUpdater(req, res) {
    ...
    async runSettingsInstaller(req, res) {
    ...
    async getParamValue(req, res) {
        logger.debug('Obteniendo valor de un parámetro');
        const param_name = req.query.param;
        try {
            if (!param_name) {
    ...
    async saveSettings(req, res) {
        ...
        try {
            ...
            const settings_map = new Map(Object.entries(body));
            const settingsService = new SettingsService(new SettingsSqliteDatasource(SETTINGS_PATH));
