...
import { SettingsDatasource } from "../../data/datasources/settings.datasource.js";
import { logger } from "../../helpers/logger-helper.js";
import { SettingsFile } from "../entities/setting.js";

export class SettingsService {
  // Caché de ajustes, para no tener que acceder a la base de datos todo el rato.
  settingsCache: Map<string, any>;

  constructor(private readonly settingsDatasource: SettingsDatasource) {
    this.settingsCache = new Map();
  }

  async getSetting(params: { name: string }) {
    try {
  ...
  async getParamValue(params: { name: string, cache?: boolean }): Promise<any | undefined> {
    try {
      if(params.cache) {
  ...
  async saveSettings(params: {settings_map: Map<string, string>}): Promise<boolean> {
    try {
      ...
      const settings_types: Map<string, string> = new Map();
      ...
      for(const [name, value] of settings_map) {
        ...
        switch(type) {
          ...
          case 'int':
            const intVal = parseInt(value);
            if(!isNaN(intVal)) valid = true;
            this.settingsCache.set(name, intVal);
            break;
          case 'float':
            const floatVal = parseFloat(value);
            if(!isNaN(floatVal)) valid = true;
            this.settingsCache.set(name, floatVal);
            break;
          case 'boolean':
            if(value == "true" || value == "false") valid = true;
        ...
        if(valid) await this.settingsDatasource.modifySetting(name, value);
        else logger.error('El dato introducido no tiene un tipo válido');
      }
      return true;
    } catch(error) {
  ...
  async saveSetting(name: string, value: string): Promise<boolean> {
    try {
  ...
  async runSettingsInstaller() {
    try {
      ...
      // Parse the file.
      const parsed_settings: SettingsFile = JSON.parse(settings_file);
