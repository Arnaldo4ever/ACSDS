...
export class MetafieldsController {
  ...
  public async getMetafields(req: Request, res: Response) {
    ...
    try {
      ...
      const result = {
        ...
        missing_metafields: [] as MetafieldDefinition[],
        metaobjects: [] as MetaobjectDefintion[],
        missing_metaobjects: [] as MetaobjectDefintion[]
      };
  ...
  public async createMetafields(req: Request, res: Response) {
    ...
    try {
      ...
      const metaobjectsRepository = new MetaobjectsRepository(new MetaobjectsShopifyDatasource(shop));
      const metaobjectsService = new MetaobjectsService(metaobjectsRepository);
      const metaobjectMaps: Map<string, string> = new Map();
      for(let i = 0; i < REQUIRED_METAOBJECTS.length; i++) {
          ...
          metafieldDefinition.validations.forEach((validation) => {
            ...
            switch (validation.name) {
              case "metaobject_definition_id":
                ...
                if(metaobjectDefinitionId) validation.value = metaobjectDefinitionId;
                break;
              case "metaobject_definition_ids":
                ...
                const result: string[] = [];
                validation.value.forEach((val: string) => {
      ...
      logger.error(
        "Error al intentar crear los metafields de la aplicación, ",
        error
      );
      res.status(500).json({ status: "Server Error" });
      return;
