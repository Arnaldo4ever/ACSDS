...
import { SettingsDatasource } from "../settings.datasource.js";
import { SQLiteBaseDatasource } from "./sqlite-base.datasource.js";

export class SettingsSqliteDatasource extends SQLiteBaseDatasource implements SettingsDatasource {
  ...
  async deleteSetting(setting: Setting): Promise<boolean> {
    const db = this.getDb();
    try {
      // El authenticate simplemente comprueba que podemos acceder a la base de datos.
      await db.authenticate();
  ...
  async insertSetting(setting: Setting): Promise<boolean> {
    const db = this.getDb();
    try {
      // El authenticate simplemente comprueba que podemos acceder a la base de datos.
      await db.authenticate();
  ...
  async getSetting(name: string): Promise<Setting | undefined> {
    ...
    try {
      ...
      const setting_name = setting.get('name') as string;
      const description = setting.get('description') as string;
      const type = setting.get('type') as string;
  ...
  async modifySetting(name: string, value: string): Promise<boolean> {
    const db = this.getDb();
    try {
      // El authenticate simplemente comprueba que podemos acceder a la base de datos.
      await db.authenticate();
  ...
  async getSettingsData(): Promise<Setting[] | undefined> {
    const db = this.getDb();
    try {
      await db.authenticate();
      const SettingsModel = this.getSettingsModel(db);
